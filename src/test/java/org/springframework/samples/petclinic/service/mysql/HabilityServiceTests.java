...
public class HabilityServiceTests {
	...
	@ParameterizedTest
	@CsvSource({
		"8000, Circuit Test, 2020-04-16, Hability ConTEST, 1000", "8000, Circuit Test, 2020-06-08, Hability ConTEST 2, 1000"
	})
	public void addNewHabilityContest(final Integer capacity, final String circuit, final LocalDate date, final String name, final Double rewardMoney) {
		...
		try {
			...
			Assertions.assertThat(hability.getId()).isNotNull();
	...
	public void checkFirstPrize() {
		Hability hability = this.habilityRepo.findById(4).get();
		Assertions.assertThat(hability.getFirstClassified()).isEqualTo(hability.getRewardMoney() * 0.5);
	}

	@Test
	public void checkSecondPrize() {
		Hability hability = this.habilityRepo.findById(4).get();
		Assertions.assertThat(hability.getSecondClassified()).isEqualTo(hability.getRewardMoney() * 0.35);
	}

	@Test
	public void checkThirdPrize() {
		Hability hability = this.habilityRepo.findById(4).get();
		Assertions.assertThat(hability.getThirdClassified()).isEqualTo(hability.getRewardMoney() * 0.15);
	}
	...
	public void editHabilityContestSuccess() throws ReservedDateExeception, SponsorAmountException, JudgeNotFoundException {
		...
		Assertions.assertThat(this.habilityService.findHabilityById(4).getCapacity()).isEqualTo(1000);
	}
	...
	public void editHabilityDateException() {
		...
		Assertions.assertThat(exeception).isEqualTo(ReservedDateExeception.class.getName());
	}
	...
	public void editHabilityWithSponsorsAndJudge(final String status) throws ReservedDateExeception, SponsorAmountException, JudgeNotFoundException, DataAccessException, DuplicatedSponsorNameException {
		Hability hability = this.habilityService.findHabilityById(4);
		...
		Assertions.assertThat(habilityAssert.getSponsors().stream().mapToDouble(x -> x.getMoney()).sum()).isGreaterThanOrEqualTo(7000.00);
		Assertions.assertThat(habilityAssert.getJudges()).isNotEmpty();
		Assertions.assertThat(habilityAssert.getStatus()).isEqualTo(status);
	}
	...
	public void editHabilitySponsorAmountException(final Double money) throws DataAccessException, ReservedDateExeception, JudgeNotFoundException, DuplicatedSponsorNameException {
		...
		try {
			...
			Logger.getLogger(HabilityServiceTests.class.getName()).log(Level.SEVERE, null, ex);
			Hability habilityAssert = this.habilityService.findHabilityById(4);
			Assertions.assertThat(habilityAssert.getSponsors().stream().mapToDouble(x -> x.getMoney()).sum()).isLessThan(7000.00);
		}
	}

	@Test
	public void editHabilityJudgeException() throws ReservedDateExeception, SponsorAmountException, DataAccessException, DuplicatedSponsorNameException {
		Hability hability = this.habilityService.findHabilityById(4);
		hability.setStatus("PENDING");
		...
		try {
			...
			Logger.getLogger(HabilityServiceTests.class.getName()).log(Level.SEVERE, null, ex);
			Hability habilityAssert = this.habilityService.findHabilityById(4);
			Assertions.assertThat(habilityAssert.getJudges()).isEmpty();
		}
	}
