...
class WinnersUpdate extends Simulation {
	...
	object DeleteLucky {
		val deleteLucky = exec(http("DeleteLucky")
			.get("/tournaments/race/1/result/4/delete")
			.headers(headers_0))
		.pause(12)
	}

	object DeletePongo {
		val deletePongo = exec(http("DeletePongo")
			.get("/tournaments/race/1/result/10/delete")
			.headers(headers_0))
		.pause(12)
	}
	...
	val scnPositive = scenario("WinnersUpdatePositive").exec(Home.home,
													Login.login,
													Tournaments.tournaments,
													FirstRace.firstRace,
													ShowResults.showResults,
													DeletePongo.deletePongo,
													FindOwners.findOwners,
													FindOwner.findOwner,
													CarlosEsteban.carlosEsteban,
													AddVisitPongo.addVisitPongo)

	val scnNegative = scenario("WinnersUpdateNegative").exec(Home.home,
													Login.login,
													Tournaments.tournaments,
													FirstRace.firstRace,
													ShowResults.showResults,
	...
	setUp(
		scnPositive.inject(rampUsers(5000) during (50 seconds)),
		scnNegative.inject(rampUsers(5000) during (50 seconds))
